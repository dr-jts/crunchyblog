...
PostGIS is a popular backend for mapping technology, so there are many options to choose from to create maps.  Data can be rendered to a raster image using a web map server like [GeoServer](https://geoserver.org/) or [MapServer](https://mapserver.org/); it can be converted to GeoJSON or vector tiles via servers such as [`pg_featureserv`](https://github.com/CrunchyData/pg_featureserv) and [`pg_tileserv`](https://github.com/CrunchyData/pg_tileserv) and then shipped to a Web browser for rendering by a library such as [OpenLayers](https://openlayers.org/), [MapLibre](https://maplibre.org/) or [Leaflet](https://leafletjs.com/); or a GIS application such as [QGIS](https://qgis.org) can connect to the database and create richly-styled maps from spatial queries.  
...
While PostGIS has had the function [`ST_AsSVG`](https://postgis.net/docs/manual-3.3/ST_AsSVG.html) for years, it only produces the SVG [**path data**](https://svgwg.org/svg2-draft/paths.html#PathData) attribute value.  Much more is required to create a fully-styled SVG document.
...
For examples of using it to create geometric and mathematical designs see the `pg-svg` [`demo`](https://github.com/dr-jts/pg_svg/tree/master/demo) folder.  Here's an image of a Lissajous knot generated by [this SQL](https://github.com/dr-jts/pg_svg/blob/master/demo/math/lissajous-knot-34-svg.sql).

![Lissajous Knot](lissajous-knot-34.svg)
